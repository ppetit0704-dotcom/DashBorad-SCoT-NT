...
def calcul_autofinancement(df, budget):
    """
    Calcule l'auto-financement pour un budget donné.
    
    Args:
        df (DataFrame): Le DataFrame contenant les données.
        budget (str): Le libellé du budget sélectionné.
    
    Returns:
        dict: Les indicateurs financiers formatés.
    """
    # Filtre sur le budget sélectionné
    df_budget = df[df["Libellé_budget"] == budget]

    # Sommes par chapitre pour le budget filtré
    ...
    epargne_nette = epargne_brute - sum(sommes.get(c, 0) for c in ["16"])

    # Report N-1 sur le même budget
    report_n1 = sum(sommes.get(c, 0) for c in ["002"])#sommes.get("002", 0)
